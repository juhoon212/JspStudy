...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
    // 스레드가 이동되어 실행되는 JSP 문서에서는 request 객체의 정보를 사용할 수 있지만 변경 불가
    // request 객체의 정보를 변경해야될 경우 요청 JSP 문서에서 처리
    String master = request.getParameter("master");
    // request.setCharacterEncoding("utf-8");
    
    
    // 스레드가 이동되어 실행되는 JSP문서에서는 response 객체에 실행 결과를 포함하여 요청
    // JSP 문서에 전달 할 수 있지만 클라이언트에게 에러코드나 URL주소를 직접 전달하여 응답 불가능
